...
#~
    ...
    def saveDownloaders(self, transmission_order=None, transmission_basedir=None, transmission_ratio=None, transmission_password=None,
        ...
        try:
            ...
            for curDownloaderStr in downloader_str_list:
                ...
                if curEnabled and curDownloader == 'transmission':
                    ...
                    if not len(transmission_host) > 0:
                        results.append("Transmission disabled! 'host' is empty!")
                        sickbeard.TRANSMISSION = curEnabled = False
                    if not len(transmission_port) > 0:
                        results.append("Transmission disabled! 'port' is empty!")
                        sickbeard.TRANSMISSION = curEnabled = False
                    if not len(transmission_basedir) > 0:
                        results.append("Transmission disabled! 'base dir' is empty!")
                        sickbeard.TRANSMISSION = curEnabled = False
                    if not (transmission_basedir[-1:] in ['/', '\\']):
                        results.append("Transmission disabled! 'base dir' needs a trailing slash!")
                        sickbeard.TRANSMISSION = curEnabled = False
            ...
            logger.log(str(err), logger.DEBUG)
